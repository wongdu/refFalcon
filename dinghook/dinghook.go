...
// SendActionCard 发送actioncard格式消息
func (ding Ding) SendActionCard(message ActionCard) Result {
	return ding.Send(message)
}
...
func (ding Ding) Send(message interface{}) Result {
	...
	if m, ok := message.(Message); ok {
		...
		paramsMap = convertMarkdown(*m)
	} else if m, ok := message.(ActionCard); ok {
		paramsMap = convertActionCard(m)
	} else if m, ok := message.(*ActionCard); ok {
		paramsMap = convertActionCard(*m)
	} else {
		return Result{ErrMsg: "not support message type"}
...
func convertActionCard(m ActionCard) map[string]interface{} {
	var paramsMap = make(map[string]interface{})
	paramsMap["msgtype"] = "actionCard"
	paramsMap["actionCard"] = map[string]string{"title": m.Title, "text": m.Content, "hideAvatar": "0", "btnOrientation": "0",
		"singleTitle": "阅读全文", "singleURL": m.CompleteContentURL}
	return paramsMap
}
